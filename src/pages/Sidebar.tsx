...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    const newChecked = courseLessons.reduce<Record<string, boolean>>((acc, lesson) => {
      acc[lesson.id] = Array.isArray(lesson.topicCheckbox) ? lesson.topicCheckbox[0]?.completed || false : false;
      return acc;
    }, {});

    // Update state only if there's a difference
    setCheckedLessons((prev) => (JSON.stringify(prev) === JSON.stringify(newChecked) ? prev : newChecked));
  }, [courseLessons]);
  ...
  const lessonItems = useMemo(() => courseLessons?.map((lesson: Lesson) => (
    <div
      key={lesson.id}
    ...
    <div className="fixed inset-y-0 left-0 lg:static w-auto rounded-lg bg-white dark:bg-darkSecondary transform transition-all duration-300 ease-in-out z-40">
      <div className="p-4 bg-gray-50 dark:bg-darkSecondary border-gray-200 rounded-lg flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
          Course Lessons ({Object.keys(checkedLessons).length})
        </h2>
      </div>
