...
export const fetchBounties = async (courseId: string): Promise<Bounty> => {
  try {
    const response = await apiClient.get<ApiResponse<Bounty>>(
      `/bounties/bounty/${courseId}`,
    );
...
//REG FOR BOUNTY
export const bountyApplication = async (
  bountyId: string,
  courseId: string,
): Promise<Bounty> => {
  try {
    const response = await apiClient.post<ApiResponse<Bounty>>(
      `/bounties/bounty-application/${bountyId}`,
      { courseId },
    );
...
export const getAppliedBounties = async (
  courseId: string,
): Promise<AppliedBounty> => {
  try {
    const response = await apiClient.get<ApiResponse<AppliedBounty>>(
      `/bounties/bounty-application/${courseId}`,
    );
