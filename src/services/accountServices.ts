...
export const forgotOtpService = async (
  email: string,
): Promise<ForgotOtpResponse> => {
  try {
    const response = await apiClient.post<ApiResponse<ForgotOtpResponse>>(
      "/dashboardUsers/user/send-forgot-otp",
      { email },
    );
...
// OTP FOR FORGOT PASSWORD
export const forgotEmailVerifyService = async (
  email: string,
  otp: string,
  ...
  try {
    ...
    const response = await apiClient.post<ApiResponse<EmailVerifyResponse>>(
      "/dashboardUsers/user/verify-forgot-otp",
      { email, otp },
    );
    if (!response.data.additional) {
      throw new Error("Data is undefined");
